...
class Link_prediction():
    ...
    def get_test_and_train(self, is_weighted=False):
        is_weighted=True
        cc_problem = False
    ...
    def get_test_and_train(self, is_weighted=False):
        ...
        if is_weighted:
            for idx, (u, v) in enumerate(G.edges()):
                G.edges[u,v]["weight"] = self.counts[idx].item()
        ...
        for i in range(len(idx_i_test)):
            ...
            target_nodes = G.neighbors(int(idx_i_test[i]))
            if int(idx_j_test[i]) in target_nodes:  # Loop through neighbors (super fast instead of self.edge_list)
                temp_weight = G.edges[int(idx_i_test[i]), int(idx_j_test[i])]["weight"]
                G.remove_edge(int(idx_i_test[i]), int(idx_j_test[i]))
                if nx.number_connected_components(G) == 1:
                    target.append(1)
                else:
                    G.add_edge(int(idx_i_test[i]),
                               int(idx_j_test[i]))  # skip the draw if the link splits network into two components
                    G.edges[int(idx_i_test[i]),int(idx_j_test[i])]["weight"] = temp_weight
                    cc_problem = True
                    delete_idx.append(i)
                    continue
            else:
        ...
        for idx in range(len(temp)):
            ...
            edge_list[1, idx] = temp[idx].split()[1]
        self.edge_list = torch.from_numpy(edge_list).long()
        self.counts = torch.from_numpy(np.array(list(nx.get_edge_attributes(G, "weight").values())))
        
        #if is_weighted:
        #    temp_counts = []
        #    count_dict = nx.get_edge_attributes(G, "weight")
        #    for i in range(self.edge_list.shape[1]):
        #        temp_counts.append(count_dict[(self.edge_list[:,i][0].item(), self.edge_list[:,i][1].item())])
        #    self.counts = torch.from_numpy(np.array(temp_counts))
        self.G = G
