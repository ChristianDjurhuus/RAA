...
class RAA(nn.Module):
    ...
    def log_likelihood(self):
        ...
        z_dist = (((torch.unsqueeze(torch.matmul(self.latent_zi, torch.matmul(F.softmax(self.Z, dim=1), F.softmax(self.C,dim=1))), 1) -
         torch.matmul(self.latent_zj, torch.matmul(F.softmax(self.Z, dim=1), F.softmax(self.C, dim=1))) + 1e-06 )**2 ).sum(-1))**0.5
