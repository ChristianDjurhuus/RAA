...
class BDRRAA(nn.Module, Link_prediction, Visualization):
    def __init__(self, k, d,partition_size, sample_size, data, data_type = "sparse", data2 = None, non_sparse_i = None, non_sparse_j = None, sparse_i_rem = None, sparse_j_rem = None, missing=None):
        ...
        self.sampling_i_weights = torch.ones(self.partition_size[0], device = self.device)
        self.sampling_j_weights = torch.ones(self.partition_size[1], device = self.device)

        self.sample_i_size = int(self.partition_size[0] * self.sample_size)
        self.sample_j_size = int(self.partition_size[1] * self.sample_size)

        self.beta = torch.nn.Parameter(torch.randn(self.partition_size[0], device = self.device))
        self.gamma = torch.nn.Parameter(torch.randn(self.partition_size[1], device = self.device))
        self.softplus = nn.Softplus()
        self.A = torch.nn.Parameter(torch.randn(self.d, self.k, device = self.device))

        self.Z_i = torch.nn.Parameter(torch.randn(self.k, self.partition_size[0], device = self.device))
        self.Z_j = torch.nn.Parameter(torch.randn(self.k, self.partition_size[1], device = self.device))

        self.Gate = torch.nn.Parameter(torch.randn(self.partition_size[0] + self.partition_size[1], self.k, device = self.device))

        self.losses = []
        self.N = self.partition_size[0]+self.partition_size[1]
    ...
    def sample_network(self):
        ...
        indexC, valueC = spspmm(edges, torch.ones(edges.shape[1], device=self.device), indices_j_translator,
                                                        torch.ones(indices_j_translator.shape[1], device = self.device), self.partition_size[0], self.partition_size[1],
                                                        self.partition_size[1], coalesced=True)
        ...
        indexC, valueC = spspmm(indices_i_translator, torch.ones(indices_i_translator.shape[1], device = self.device), indexC, valueC,
                                self.partition_size[0], self.partition_size[0], self.partition_size[1], coalesced=True)
        ...
        if self.missing!=None:
            ...
            indexC, valueC = spspmm(self.missing.T, torch.ones(self.missing.T.shape[1]), indices_j_translator,
                                    torch.ones(indices_j_translator.shape[1]), self.partition_size[0], self.partition_size[1],
                                    self.partition_size[0], coalesced=True)
            # second matrix multiplication C = Indices translator x B, indexC returns where we have edges inside the sample
            ...
            return sample_i_idx, sample_j_idx, sparse_i_sample, sparse_j_sample, non_sparse_i_sample_removed, non_sparse_j_sample_removed
    ...
    def log_likelihood(self):
        if self.missing==None:
            ...
            sample_i_idx, sample_j_idx, sparse_sample_i, sparse_sample_j, non_sparse_i_sample_removed, non_sparse_j_sample_removed = self.sample_network()
        ...
        #Gate = torch.cat((self.Gate, self.Gate), 1)
        Gate = torch.sigmoid(self.Gate)  # Sigmoid activation function
        ...
        if self.missing!=None:
            bias_matrix_missing = self.beta[non_sparse_i_sample_removed] + self.gamma[non_sparse_j_sample_removed]
            mat_missing = (torch.exp(bias_matrix_missing -
                         ((torch.mm(AZC,Z_i[:,non_sparse_i_sample_removed]).T -
                           torch.mm(AZC,Z_j[:,non_sparse_j_sample_removed]).T + 1e-06) ** 2).sum(-1) ** 0.5)).sum()
        ...
        log_likelihood_sparse = mat_links - (mat - mat_missing)
        return log_likelihood_sparse

    def train(self, iterations, LR = 0.1, print_loss = False):
        optimizer = torch.optim.Adam(params = self.parameters(), lr=LR)
        for _ in range(iterations):
            loss = - self.log_likelihood() / ((self.partition_size[0]*self.sample_size)+(self.partition_size[0]+self.sample_size))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
