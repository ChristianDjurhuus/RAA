...
class DRAA(nn.Module):
    def __init__(self, X, input_size, k, d):
    ...
    def log_likelihood(self):
        ...
        M = torch.matmul(self.A, torch.matmul(torch.matmul(Z, C), Z)).T # (N x d) 
        z_dist = ((M.unsqueeze(1) - M + 1e-06)**2).sum(-1)**0.5 # (N x N)
        theta = beta - z_dist #(N x N)
        softplus_theta = F.softplus(theta) # log(1+exp(theta))
        LL = 0.5 * (theta * self.X).sum() - 0.5 * torch.sum(softplus_theta-torch.diag(torch.diagonal(softplus_theta))) #Times by 0.5 to avoid double counting
...
if __name__ == "__main__": 
    ...
    if link_pred:
        ...
        for i in range(len(idx_i_test)):
            idx_j_test[i] = torch.arange(idx_i_test[i].item(), float(X_shape[1]))[torch.multinomial(input = torch.arange(idx_i_test[i].item(), float(X_shape[1])), num_samples=1, replacement=True).item()].item() #Temp solution to sample from upper corner
    ...
    model = DRAA(X = X, input_size = X.shape, k=k, d=d)
