...
LSM_AUC_score_plot = {key: [] for key in data_names}
LSM_KNN_plot = {key: [] for key in data_names}
LSM_LR_plot = {key: [] for key in data_names}
...
for i, data in tqdm(enumerate(dataset)):
    ...
    for seed in seeds: # Train model for each seed
        ...
        LSM_AUC_score.append(LSM_AUC_score)
    ...
    LSM_AUC_score_plot[data_names[i]].append(LSM_AUC_score)
    LSM_KNN_plot[data_names[i]].append(LSM_KNN)
    LSM_LR_plot[data_names[i]].append(LSM_LR)
...
for data_name in data_names:
    ax1.plot(LSM_AUC_scores, label = data_name + "LSM AUC")
    ax1.plot(AA_AUC_scores[data_name], label = data_name + "AA AUC")
ax1.set_xticks(n_archetypes.numpy())
ax1.set_title(f"DRRAA vs. LSM with {iterations} iterations")
ax1.set_ylabel("AUC-score")

for data_name in data_names:
    ax2.plot(LSM_KNN_plot[data_name], label = data_name + "LSM KNN")
    ax2.plot(LSM_LR_plot[data_name], label = data_name + "LSM LR")
    ax2.plot(AA_KNN_plot, label = data_name + "AA KNN")
    ax2.plot(AA_LR_plot, label = data_name + "AA LR")
