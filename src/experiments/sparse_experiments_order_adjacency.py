...
def get_ordered_adjacency(datasets, ks, sample_size, iterations, LR, print_loss = False):
    ...
    for idx, dataset in enumerate(datasets):
        ...
        for k in ks:
            ...
            raa.order_adjacency_matrix(filename = f"ordered_adjacency_{dataset}_k{k}.pdf", show = False)

if __name__ == "__main__":
    datasets = ["cora", "facebook", "grqc", "hepth", "astroph", "dblp", "amazon"]
    # Set iterations for each dataset
    iterations = [10000, 10000, 10000, 10000, 15000, 20000, 35000]
    # Set sample procentage for each dataset
    sample_size = [0.5, 0.5, 0.5, 0.5, 0.5, 0.2, 0.05]
