...
def sparse_experiments(datasets, ks, sample_size, iterations, LR, print_loss = False):
    ...
    for idx, dataset in enumerate(datasets):
        ...
        for model_num in range(TRAIN_NUMS):
            ...
            for k in ks:
                ...
                kaa.train(1000, LR = LR, print_loss = print_loss)
                ...
                RAA_AUC, _, _ = kaa.link_prediction()
                RAA_AUC_SCORES[dataset, k].append(RAA_AUC)

                kaa.embedding_density(filename = f"{dataset}_RAA_init_embedding_density_k{k}.pdf", show = False)
    ...
    with open(f"RAA_init_auc_{dataset}.txt", "w") as data:
        for k in ks:
