...
ax.plot(alphas, list(avgNMIs.values()), '-o', label="RAA", color='#e3427d')
ax.plot(alphas, [x for (x,y) in conf_NMIs.values()], '--', color='#e3427d')
ax.plot(alphas, [y for (x,y) in conf_NMIs.values()], '--', color='#e3427d')
...
ax.plot(alphas, [y for (x,y) in conf_NMIs_nr.values()], '--', color='#5d4b20')
ax.fill_between(alphas,
                 y1 = [x for (x,y) in conf_NMIs_nr.values()],
                 y2 = [y for (x,y) in conf_NMIs_nr.values()], color='#5d4b20', alpha=0.2)

ax.plot(alphas, list(avgNMIs_bare.values()), '-o', label="RAA no gating / random effects", color='#f0c808')
ax.plot(alphas, [x for (x,y) in conf_NMIs_bare.values()], '--', color='#f0c808')
ax.plot(alphas, [y for (x,y) in conf_NMIs_bare.values()], '--', color='#f0c808')
ax.fill_between(alphas,
                 y1 = [x for (x,y) in conf_NMIs_bare.values()],
                 y2 = [y for (x,y) in conf_NMIs_bare.values()], color='#f0c808', alpha=0.2)
