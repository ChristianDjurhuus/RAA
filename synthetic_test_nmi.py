...
ax.plot(alphas, list(avgNMIs.values()), '-o', label="RAA", color='#C4000D')
ax.plot(alphas, [x for (x,y) in conf_NMIs.values()], '--', color='#C4000D')
ax.plot(alphas, [y for (x,y) in conf_NMIs.values()], '--', color='#C4000D')
ax.fill_between(alphas,
                 y1 = [x for (x,y) in conf_NMIs.values()],
                 y2 = [y for (x,y) in conf_NMIs.values()], color='#C4000D', alpha=0.2)

ax.plot(alphas, list(avgNMIs_ng.values()), '-o', label="RAA no gating", color='#FF5924')
ax.plot(alphas, [x for (x,y) in conf_NMIs_ng.values()], '--', color='#FF5924')
ax.plot(alphas, [y for (x,y) in conf_NMIs_ng.values()], '--', color='#FF5924')
ax.fill_between(alphas,
                 y1 = [x for (x,y) in conf_NMIs_ng.values()],
                 y2 = [y for (x,y) in conf_NMIs_ng.values()], color='#FF5924', alpha=0.2)

ax.plot(alphas, list(avgNMIs_nr.values()), '-o', label="RAA no random effects", color='#FFA6BD')
ax.plot(alphas, [x for (x,y) in conf_NMIs_nr.values()], '--', color='#FFA6BD')
ax.plot(alphas, [y for (x,y) in conf_NMIs_nr.values()], '--', color='#FFA6BD')
ax.fill_between(alphas,
                 y1 = [x for (x,y) in conf_NMIs_nr.values()],
                 y2 = [y for (x,y) in conf_NMIs_nr.values()], color='#FFA6BD', alpha=0.2)

ax.plot(alphas, list(avgNMIs_bare.values()), '-o', label="RAA no gating / random effects", color='#FF2459')
ax.plot(alphas, [x for (x,y) in conf_NMIs_bare.values()], '--', color='#FF2459')
ax.plot(alphas, [y for (x,y) in conf_NMIs_bare.values()], '--', color='#FF2459')
ax.fill_between(alphas,
                 y1 = [x for (x,y) in conf_NMIs_bare.values()],
                 y2 = [y for (x,y) in conf_NMIs_bare.values()], color='#FF2459', alpha=0.2)
